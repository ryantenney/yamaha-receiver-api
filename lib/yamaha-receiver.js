...
exports.initIP = async function(providedIP){
	receiverAddress = "http://"+providedIP;
	// update the API URIs to the legit values
	receiverStatusURI = yamahaAPIURI;
	receiverVolumeURI = yamahaAPIURI;
	receiverPowerURI = yamahaAPIURI;
	logger.info("Found Yamaha target with IP ("+providedIP+")");
	retVal = {status:200, msg: "OK"};
};
...
function cleanJSON(json) {
	...
	for (const [key, value] of Object.entries(json)) {
		var result = cleanJSON(value);
		output[key.toLowerCase()] = result;
	}
	return output;
}
	...
	return new Promise(function(resolve, reject){			
		try{
			request({
				...
				var basicInfo = JSON.parse(formatter.xml2json(responseBody,{compact:true}));
				basicInfo = basicInfo.YAMAHA_AV;
				delete basicInfo._attributes;
				basicInfo = cleanJSON(basicInfo);
				resolve({status:200,msg:"OK",basicInfo});
			}).catch(function(err){
